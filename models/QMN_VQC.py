...
from layers.quantumnn.ket import QKet
...
class QMN_VQC(nn.Module):
    ...
    def forward(self, in_modalities):
        ...
        for l in range(self.num_layers):
            ...
            h_r = torch.stack(batch_size*[torch.ones(self.embed_dim)/self.embed_dim],dim =0)
        ...
        output = []
        states = [self.outer(s) for s in in_states]     
        for _h in states:
